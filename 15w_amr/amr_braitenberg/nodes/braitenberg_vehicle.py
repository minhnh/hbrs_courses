...
class BraitenbergVehicleNode:
    ...
    def _sonar_callback(self, ranges_msg):
        ...
        """
        ws = WheelSpeeds()
        r1 = ranges_msg.ranges[0]
        r2 = ranges_msg.ranges[1]

        # Normalize sonar values
        sonar1 = (r1.range - r1.min_range) / (r1.max_range - r1.min_range)
        sonar2 = (r2.range - r2.min_range) / (r2.max_range - r2.min_range)

        # Calculate speeds
        ws.speeds = self._vehicle.compute_wheel_speeds(sonar1, sonar2)

        # Publish to _wheel_speeds_publisher
    ...
    def _reconfigure_callback(self, config, params):
        ...
        # Update vehicle type and motor factors
