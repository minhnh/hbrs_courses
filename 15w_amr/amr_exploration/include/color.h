...
/** A structure which represents an RGB-encoded color.
  *
  * Provides a number of pre-defined colors and openators to cast the color to
  * a PCL-style float or a ROS-style message. */
struct Color
{
  ...
  /** Construct a color from three floats in [0...1] range. */
  Color(float nr, float ng, float nb) : r(nr * 255), g(ng * 255), b(nb * 255) { }

  /** Construct a color from three bytes in [0...255] range. */
