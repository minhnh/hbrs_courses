...
public class SamplePose2D extends SamplePose3D {
	...
	public void recalculateWeight(SamplePose2D actualPose) {
		float gaussianX = (float) calculateGaussian(this.getX(), actualPose.getX(), SamplePose2D.sigmaLinear);
		float gaussianY = (float) calculateGaussian(this.getY(), actualPose.getY(), SamplePose2D.sigmaLinear);
		float gaussianAlpha = (float) calculateGaussian(this.getAlpha(), actualPose.getAlpha(),
				SamplePose2D.sigmaAngular);
		this.setWeight(gaussianX * gaussianY * gaussianAlpha);
	}

	@Override
	public String toString() {
		return Float.toString(this.getX()) + "," + Float.toString(this.getY()) + "," + Float.toString(this.getAlpha())
