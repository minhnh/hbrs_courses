...
public class SamplePose3D {
	...
	public static class SampleGeneratorClass implements SampleGenerator<SamplePose3D> {
		...
		public SamplePose3D createUniformSample(int sampleCount, Random r) {
			...
			do {
				float x = SamplePose3D.X_MIN + r.nextFloat() * (SamplePose3D.X_MAX - SamplePose3D.X_MIN);
				float y = SamplePose3D.Y_MIN + r.nextFloat() * (SamplePose3D.Y_MAX - SamplePose3D.Y_MIN);
				float z = SamplePose3D.Z_MIN + r.nextFloat() * (SamplePose3D.Z_MAX - SamplePose3D.Z_MIN);
				float alpha = SamplePose3D.ALPHA_MIN
						+ r.nextFloat() * (SamplePose3D.ALPHA_MAX - SamplePose3D.ALPHA_MIN);
				float beta = SamplePose3D.BETA_MIN + r.nextFloat() * (SamplePose3D.BETA_MAX - SamplePose3D.BETA_MIN);
		...
		public SamplePose3D createGaussianSample(int sampleCount, Random r) {
			...
			float w = 1.0f / sampleCount;
			do {
				SamplePose3D meanSample = meanSamples[r.nextInt(meanSamples.length)];
